...
if(PHP_SAPI == 'cli' && isset($argv[0]) && $argv[0] == basename(__FILE__))
{
    ...
    $IssueClient = ThriftClient::instance('IssueService');
    //=== 发放一个
    // (1: i32 pkgid, 2: string user, 3: string ip, 4: string ext, 5: i32 count),
    $give_result = $IssueClient->give(1, '123456', '192.168.0.1', 'ext_info', 1);
    var_export($give_result);
    //=== 获得列表
    $pkgIdList= array(1, 2);
    $list = $IssueClient->listPkg($pkgIdList);
    var_export($list);
    exit;
